...
//     } catch (err) {
//       return res.status(400).json({ success: false, message: "Invalid tag or instructions format" })
//     }
...
exports.editCourse = async (req, res) => {
  try {
    ...
    // Remove courseId from updates to avoid modifying it
    delete updates.courseId;

    // Find the course first to ensure it exists
    const course = await Course.findById(courseId);
    if (!course) {
      return res.status(404).json({ 
        success: false, 
    ...
    if (req.files && req.files.thumbnailImage) {
      ...
      const thumbnailImage = await uploadImageToCloudinary(
        thumbnail,
        process.env.FOLDER_NAME.trim() // Ensure no whitespace
      );
      updates.thumbnail = thumbnailImage.secure_url;
    }

    // Remove instructor from updates to prevent changing the instructor
    // and to avoid validation errors (instructor is required in schema)
    ...
    const updatedCourse = await Course.findByIdAndUpdate(
      ...
      { 
        new: true, 
        runValidators: true // Keep validation for other fields
      }
    )
      .populate({
        path: "instructor",
...
// course ko instructor me show krna!
...
//     // Find all courses belonging to the instructor
//     //Instructor data recently bala chahiye 
...
//     console.log("Found courses:", instructorCourses);
//     console.log("Number of courses:", instructorCourses.length);
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    // courseSection/courseContent  me se SectionId ! 
    for (const sectionId of courseSections) {
      ...
      if (section) {
        ...
        //Section se sub-section me se subsection Id ! 
